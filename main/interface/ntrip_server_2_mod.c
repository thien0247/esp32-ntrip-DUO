...
static const char *TAG = "NTRIP_SERVER2";
...
static EventGroupHandle_t server_2_event_group;

static status_led_handle_t status_led = NULL;
static stream_stats_handle_t stream_stats = NULL;

static TaskHandle_t server_2_task = NULL;
static TaskHandle_t sleep_task = NULL;

static void ntrip_server_2_uart_handler(void* handler_args, esp_event_base_t base, int32_t length, void* buffer) {
    EventBits_t event_bits = xEventGroupGetBits(server_2_event_group);

    // Reset data availability bit
    if ((event_bits & DATA_READY_BIT) == 0) {
        xEventGroupSetBits(server_2_event_group, DATA_READY_BIT);
    ...
    if ((event_bits & DATA_SENT_BIT) == 0) xEventGroupSetBits(server_2_event_group, DATA_SENT_BIT);

    int sent = write(sock, buffer, length);
    if (sent < 0) {
        destroy_socket(&sock);
        vTaskResume(server_2_task);
    } else {
        stream_stats_increment(stream_stats, 0, sent);
    }
}

static void ntrip_server_2_sleep_task(void *ctx) {
    vTaskSuspend(NULL);

    while (true) {
        // If wait time exceeded, clear data ready bit
        if (data_keep_alive == NTRIP_KEEP_ALIVE_THRESHOLD) {
            xEventGroupClearBits(server_2_event_group, DATA_READY_BIT);
...
static void ntrip_server_2_task(void *ctx) {
    server_2_event_group = xEventGroupCreate();
    uart_register_read_handler(ntrip_server_2_uart_handler);
    xTaskCreate(ntrip_server_2_sleep_task, "ntrip_server_2_sleep_task", 2048, NULL, TASK_PRIORITY_INTERFACE, &sleep_task);
    ...
    while (true) {
        ...
        if ((xEventGroupGetBits(server_2_event_group) & DATA_READY_BIT) == 0) {
            ESP_LOGI(TAG, "Waiting for UART input to connect to caster");
            uart_nmea("$PESP,NTRIP,SRV_2,WAITING");
            xEventGroupWaitBits(server_2_event_group, DATA_READY_BIT, true, false, portMAX_DELAY);
        }
        ...
        uart_nmea("$PESP,NTRIP,SRV_2,CONNECTING,%s:%d,%s", host, port, mountpoint);
        ...
        uart_nmea("$PESP,NTRIP,SRV_2,CONNECTED,%s:%d,%s", host, port, mountpoint);
        ...
        // Connected
        xEventGroupSetBits(server_2_event_group, CASTER_READY_BIT);

        // Await disconnect from UART handler
        vTaskSuspend(NULL);

        // Disconnected
        xEventGroupClearBits(server_2_event_group, CASTER_READY_BIT | DATA_SENT_BIT);

        if (status_led != NULL) status_led->active = false;

        ESP_LOGW(TAG, "Disconnected from %s:%d/%s", host, port, mountpoint);
        uart_nmea("$PESP,NTRIP,SRV_2,DISCONNECTED,%s:%d,%s", host, port, mountpoint);
...
void ntrip_server_2_init() {
    ...
    xTaskCreate(ntrip_server_2_task, "ntrip_server_2_task", 4096, NULL, TASK_PRIORITY_INTERFACE, &server_2_task);
}